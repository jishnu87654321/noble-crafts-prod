...
import { ArrowLeft, Send, Mail, Twitter, Github, Linkedin } from 'lucide-react'
...
export function SignupPage() {
  ...
  return (
    <div className="w-full h-full min-h-screen bg-black flex items-center justify-center p-4">
      ...
      <div className="flex flex-col md:flex-row w-full max-w-6xl h-full min-h-[750px] gap-8 relative z-10">
        {/* Left Panel - Card Style */}
        <div className="flex-1 relative overflow-hidden rounded-[3rem] border border-white/10 bg-zinc-950 min-h-[600px] md:min-h-0">
          <div className="absolute top-8 left-8 z-20">
            <button
              onClick={() => router.push('/')}
              className="w-12 h-12 bg-black/40 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center hover:bg-black/60 transition-all group"
            >
        ...
        <div className="flex-1 p-10 md:p-14 bg-zinc-950 rounded-[3rem] border border-white/10 flex flex-col justify-center">
          <form onSubmit={handleSubmit} className="space-y-8">
            {/* Full Name */}
            <div className="space-y-3">
              <label htmlFor="name" className="block text-sm font-bold text-white ml-1">
                Full Name
              </label>
              <input
                type="text"
                ...
                className="w-full px-6 py-5 bg-white/[0.03] border border-white/10 rounded-2xl text-white focus:border-[#e78a53] outline-none transition-all placeholder:text-white/10 text-lg"
                required
              />
            </div>
            ...
            {/* Email Field */}
            <div className="space-y-3">
              <label htmlFor="email" className="block text-sm font-bold text-white ml-1">
                Email Address
              </label>
              <input
                type="email"
            ...
            {/* Phone Number */}
            <div className="space-y-3">
              <label htmlFor="phone" className="block text-sm font-bold text-white ml-1">
                Phone Number
              </label>
              <input
                type="tel"
            ...
            {/* Website Type */}
            <div className="space-y-3">
              <label htmlFor="websiteType" className="block text-sm font-bold text-white ml-1">
            ...
            <AnimatePresence>
              {formData.websiteType === "other" && (
                ...
                >
                  <label htmlFor="otherWebsiteType" className="block text-sm font-bold text-[#e78a53] ml-1">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-[#e78a53] hover:bg-[#e78a53]/90 text-white py-5 px-8 rounded-3xl font-bold text-xl shadow-xl shadow-[#e78a53]/20 transition-all hover:-translate-y-1 active:scale-[0.98] mt-4 flex items-center justify-center gap-3 group"
            >
